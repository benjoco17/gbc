    ...
    public static function input($input, $field, $value, $lead_id, $form_id)
    {
        ...
        if ($field["type"] == "sms_verification") {
            ...
            if ($is_form_editor) {
                ...
                $mobile_field_id = rgget("field_sms_verify_mobile", $field);
        ...
        if ($field["type"] == "sms_verification") {
            ...
            if (empty($mobile) || strlen($mobile) < 3) {
                ...
                if (!in_array($mobile, $white_list)) {
                    ...
                    if (empty($code) || !$code) {
                        $type = rgget('sms_verify_code_type_radio', $field);
                        if ($type == 'manual') {
                            $delimator = ',';
                            $manual = explode($delimator, rgget('sms_verify_code_type_manual', $field));
                            ...
                            $code = self::rand_mask(rgget('sms_verify_code_type_rand', $field));
                        }
                    }

                    $allowed_try = rgget('sms_verify_try_num', $field);
                    ...
                    if ($try_num <= $allowed_try && !rgempty('input_' . $field["id"]) && !empty($code) && rgpost('input_' . str_replace( '.' , '_' , $field["id"] ) ) == $code) {
                        ...
                        if ($try_num < $allowed_try) {
                            ...
                            $message = rgget('sms_verify_code_msg_body', $field);
                            ...
                            $allowed_send = rgget('sms_verify_sent_num', $field);
