...
class GFHANNANSMS_Pro_WP_SMS
{
    ...
    public static function input($input, $field, $value, $lead_id, $form_id)
    {
        ...
        if ($field["type"] == "sms_subscribtion") {
            ...
            if (!is_admin() && (rgget("wp_sms_subs", $field) == 'unsubscribe' || rgget("wp_sms_group_select", $field) == 'force')) {
                $hidden = '';
            ...
            if ($is_entry_page && rgget("wp_sms_subs", $field) == 'unsubscribe') {
            ...
            $type = rgget("wp_sms_group_type", $field) ? rgget("wp_sms_group_type", $field) : 'radio';
            ...
            if ($is_form_editor || $type == 'select') {
                ...
                if (!is_admin() && $field->enableEnhancedUI) {
                    ...
                    foreach ($form['fields'] as $field) {
                        ...
                        if ($field->enableEnhancedUI && in_array($input_type, array('select', 'multiselect', 'sms_subscribtion'))) {
            ...
            if ($is_form_editor) {
                ...
                if (rgget("wp_sms_subs", $field) == 'unsubscribe') {
                    $style .= $hide_radio;
                    $style .= $hide_select;
                    $style .= $hide_force;
                } else {
    ...
    public static function validation($result, $value, $form, $field)
    {
        ...
        if ($field["type"] == "sms_subscribtion") {
            ...
            if (rgget("wp_sms_subs", $field) != 'unsubscribe' && rgget("wp_sms_repeat_error", $field)) {
                ...
                if ($mobile_exist) {
                    ...
                    $result["message"] = rgget("wp_sms_repeat_mgs", $field) ? rgget("wp_sms_repeat_mgs", $field) : __("The entered mobile number has previously been used in this form.", "GF_SMS");
                    add_filter('wp_sms_display_none', array('GFHANNANSMS_Pro_WP_SMS', 'apply_false'));
                }

            }

            if (RGFormsModel::get_input_type($field) == 'adminonly_hidden' || rgget("wp_sms_subs", $field) == 'unsubscribe' || (rgget("wp_sms_group_select", $field) == 'force' && !$mobile_error)) {
